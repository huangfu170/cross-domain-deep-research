...
description: 面向 Claude Code 或 Codex 的端到端跨领域研究机会发现 skill。用于从目标领域学术论文中抽取有论文证据的开放问题，分析未解决原因，检索其他领域可迁移方法，执行 novelty check，设计具体 application plan，审查可行性，并生成包含研究假设和最小实验的报告。这是中文用户入口；不要让用户手动串联规划、抽取、迁移或报告流程。
---
...
这是中文用户的唯一入口。用户只需要给出目标研究领域，agent 必须在内部完成所有阶段，不要让用户自己选择或串联子流程。

## 目标

本 skill 的目标不是生成泛泛综述，而是发现可验证的研究机会：

- 从目标领域论文中抽取 open problems，重点关注 limitations、future work、open challenges、benchmark gaps、负结果和部署约束。
- 解释现有方法为什么仍未解决这些问题。
- 将问题抽象成可跨领域迁移的机制。
- 检索其他领域已经有效、但尚未直接应用到目标领域的方法。
- 对直接应用和机制等价方法执行 novelty check。
- 生成具体 application plan、研究假设、最小实验、可行性 review 和最终报告。
...
同时以检索方式动态加载迁移模式库，避免把整份 YAML 放入上下文：
...
5. 每个索引行格式为 `# @pattern id=<id> keywords=<keywords>`。只对命中的 id 读取对应 YAML 块：从该 `# @pattern` 行开始，到下一个 `# @pattern` 行之前结束。
...
每个模式只需要包含 `id`、`transfer`、`use_when`、`queries`、`apply` 和 `risks`。这些模式不是最终结论，而是用于生成候选迁移假设的启发式种子。
...
对每个 `research_question` 生成 `{output_dir}/{rq_slug}.json`。如果文件已存在，跳过以支持断点续跑。
...
对每个 `candidate_gap` 添加 `feasibility_review`：
...
- `revise`：方向有价值，但需要缩小范围、换数据、换 baseline 或调整评估。
- `hold`：关键依赖不可获得，或没有能验证核心假设的最小实验。
